...
// --- Strategy Parameters ---
const POSITION_SIDE = 'SHORT';  // 'LONG' or 'SHORT' - position type to open
const FUNDING_RATE_PREFERENCE = 'POSITIVE';  // 'POSITIVE' or 'NEGATIVE' - which funding rates to target
const FUNDING_RATE_THRESHOLD = 0;  // 0 means include all positive/negative rates
const INVESTMENT_USD = 200;
const LEVERAGE = 10;
const TAKE_PROFIT_PERCENT = 0.003;  // For LONG positions, this is the percentage above entry price
const STOP_LOSS_PERCENT = 0.001;    // For LONG positions, this is the percentage below entry price
const ENTRY_SECONDS_BEFORE_FUNDING = 15;
...
function calculateTradeMetrics(entryPrice, symbolPrecision) {
    ...
    // For LONG positions TP is higher than entry price, SL is lower than entry price
    ...
    // Verify that TP is higher and SL is lower than entry price for LONG positions
...
function displayTopCandidates(candidates) {
    console.log(chalk.blueBright(`\n--- Top Funding Rate Candidates (Top 10 by Positive Rate, Then Sorted by Next Funding Time) ---`));
    ...
    const table = new Table({
        head: [
            ...
            chalk.cyan('Net Profit ($)'), chalk.cyan('Net Loss ($)'), chalk.cyan('Short Fee ($)')
        ],
    ...
    if (!candidates || candidates.length === 0) {
        console.log(chalk.yellow("   No candidates meet the criteria currently (Positive Rate & Upcoming Funding Time)."));
        return;
    }
    ...
    candidates.forEach((c, index) => {
        ...
        // คำนวณ Funding Fee สำหรับ Short Position
        const positionValue = parseFloat(metrics.positionValue);
        const shortFundingFee = positionValue * c.fundingRate; // Positive funding rate gives positive fee for shorts
        ...
        table.push([
            ...
            shortFundingFee >= 0 ? chalk.green(shortFundingFee.toFixed(4)) : chalk.red(shortFundingFee.toFixed(4))
        ]);
    });
    console.log(table.toString());
    console.log(chalk.gray("* indicates realtime funding rate data from direct API call"));
    console.log(chalk.cyan("Short Fee ($): Amount you receive (if positive) or pay (if negative) per funding period when shorting"));
}
...
async function displayCurrentPosition() {
    ...
    try {
        ...
        // For LONG positions, PNL is calculated as (current price - entry price) * quantity
...
async function fetchAndAnalyze() {
    ...
    try {
        ...
        console.log(`Filtering/Mapping candidates based on ${FUNDING_RATE_PREFERENCE} funding rates (${FUNDING_RATE_THRESHOLD}), USDT, Valid Data...`);
        const nowTimestamp = Date.now();
        const initialCandidates = fundingRates
            .filter(rate => rate.symbol.endsWith('USDT'))
            .filter(rate => { 
                const fr = parseFloat(rate.fundingRate); 
                return !isNaN(fr) && (
                    // Filter based on funding rate preference
                    (FUNDING_RATE_PREFERENCE === 'POSITIVE' && fr >= FUNDING_RATE_THRESHOLD) || 
                    (FUNDING_RATE_PREFERENCE === 'NEGATIVE' && fr <= FUNDING_RATE_THRESHOLD)
                ); 
            })
            .map(rate => {
        ...
        // Remove duplicates - keep the one with the most extreme funding rate based on preference
        console.log("Removing duplicate symbols from candidates...");
        const uniqueSymbolMap = new Map();
        
        // For each symbol, keep only the entry with the most extreme funding rate based on preference
        initialCandidates.forEach(candidate => {
            const symbol = candidate.symbol;
            if (!uniqueSymbolMap.has(symbol) || 
                (FUNDING_RATE_PREFERENCE === 'POSITIVE' && candidate.fundingRate > uniqueSymbolMap.get(symbol).fundingRate) ||
                (FUNDING_RATE_PREFERENCE === 'NEGATIVE' && candidate.fundingRate < uniqueSymbolMap.get(symbol).fundingRate)) {
        ...
        if (realtimeRates.length > 0) {
            ...
            // Filter rates based on preference
            const filteredRates = realtimeRates.filter(item => {
                if (FUNDING_RATE_PREFERENCE === 'POSITIVE') {
                    return item.fundingRate > 0;
                } else { // NEGATIVE
                    return item.fundingRate < 0;
                }
            });
            
            // Sort by funding rate according to preference
            if (FUNDING_RATE_PREFERENCE === 'POSITIVE') {
                // Sort by highest positive funding rate first
                filteredRates.sort((a, b) => b.fundingRate - a.fundingRate);
            } else {
                // Sort by lowest negative funding rate first
                filteredRates.sort((a, b) => a.fundingRate - b.fundingRate);
            }
            
            // Show all filtered funding rates in log
            console.log(chalk.cyan(`Found ${filteredRates.length} symbols with ${FUNDING_RATE_PREFERENCE.toLowerCase()} funding rates:`));
            filteredRates.slice(0, 20).forEach((item, index) => {
            ...
            for (const item of filteredRates) {
        ...
        // Sort candidates based on funding rate preference
        if (FUNDING_RATE_PREFERENCE === 'POSITIVE') {
            // Sort by highest positive funding rate first
            initialCandidates.sort((a, b) => b.fundingRate - a.fundingRate);
            console.log(`Sorted ${initialCandidates.length} candidates by funding rate (most positive first)`);
        } else {
            // Sort by lowest negative funding rate first
        ...
        // STEP 2: Get the top candidates by funding rate
        ...
        // STEP 3: Now sort just these top ones by Next Funding Time (earliest first)
        ...
        // 6. Use the top candidates sorted by funding rate then by time
        ...
        if (topCandidate && !currentPosition && !isPlacingOrder) {
            ...
            if (IGNORE_COUNTDOWN) {
                console.log(chalk.magenta(`>>> IGNORE_COUNTDOWN is enabled. Opening ${POSITION_SIDE} position for ${topCandidate.symbol} immediately...`));
                await placeTrade(topCandidate);
            } 
            else if (timeRemainingSec > 0 && timeRemainingSec <= ENTRY_SECONDS_BEFORE_FUNDING) {
                console.log(chalk.magenta(`>>> Approaching funding time for ${topCandidate.symbol} (${timeRemainingSec.toFixed(1)}s left). Preparing ${POSITION_SIDE} entry...`));
                await placeTrade(topCandidate);
            }
...
async function placeTrade(candidate) {
    ...
    // Use position side from strategy parameters
    const side = POSITION_SIDE;
    
    console.log(chalk.blue(`\n=== Attempting ${side} Entry for ${symbol} ===`));
    ...
    try {
        ...
        // Place market order based on position side
        let entryOrder;
        if (side === 'LONG') {
            console.log(`   2. Placing MARKET BUY order for ${quantity} ${symbol}...`);
            entryOrder = await binance.futuresMarketBuy(symbol, quantity, { newOrderRespType: 'RESULT' });
            console.log(chalk.green(`      Market BUY order placed successfully.`), `Order ID: ${entryOrder.orderId}`);
        } else { // SHORT
            console.log(`   2. Placing MARKET SELL order for ${quantity} ${symbol}...`);
            entryOrder = await binance.futuresMarketSell(symbol, quantity, { newOrderRespType: 'RESULT' });
        ...
        currentPosition = {
            ...
            positionAmt: side === 'LONG' ? executedQty : -executedQty, // Positive for LONG, negative for SHORT
            entryTimestamp: entryOrder.updateTime || Date.now(),
            orderIds: { tp: null, sl: null },
            positionSide: side // Store the position side
        };
        console.log(chalk.green(`   Position Opened: ${symbol} @ ~${entryPrice}, Qty: ${executedQty}, Side: ${side}`));

        try {
            if (side === 'LONG') {
                console.log(`   3. Placing SELL LIMIT (TP) order for ${executedQty} ${symbol} @ ${tpPrice}...`);
                const tpOrder = await directPlaceLongTPOrder(symbol, executedQty, tpPrice);
        ...
        try {
            if (side === 'LONG') {
                console.log(`   4. Placing SELL STOP_MARKET (SL) order for ${executedQty} ${symbol}, Trigger @ ${slPrice}...`);
                const slOrder = await directPlaceLongSLOrder(symbol, executedQty, slPrice);
...
/**
 * Direct implementation of placing a Take Profit order for LONG positions (SELL LIMIT order)
 */
async function directPlaceLongTPOrder(symbol, quantity, price) {
    try {
        const timestamp = Date.now();
        const queryParams = `symbol=${symbol}&side=SELL&type=LIMIT&timeInForce=GTC&quantity=${quantity}&price=${price}&reduceOnly=true&timestamp=${timestamp}`;
        const signature = createSignature(queryParams);
...
/**
 * Direct implementation of placing a Stop Loss order for LONG positions (SELL STOP_MARKET order)
 */
async function directPlaceLongSLOrder(symbol, quantity, stopPrice) {
    try {
        const timestamp = Date.now();
        const queryParams = `symbol=${symbol}&side=SELL&type=STOP_MARKET&timeInForce=GTC&quantity=${quantity}&stopPrice=${stopPrice}&reduceOnly=true&timestamp=${timestamp}`;
        const signature = createSignature(queryParams);
